class Api::V1::RankMovementService
  ...
  def self.update_fran_time(stored_badge, incoming_badge, user)
    if incoming_badge[:value] >= 420
      stored_badge.update(ranking: "Beginner", ranking_value: 0, value: incoming_badge[:value])
    elsif incoming_badge[:value] < 420 && incoming_badge[:value] >= 300
      stored_badge.update(ranking: "Intermediate", ranking_value: 1, value: incoming_badge[:value])
    elsif incoming_badge[:value] < 300 && incoming_badge[:value] >= 180
      stored_badge.update(ranking: "RX", ranking_value: 2, value: incoming_badge[:value])
    elsif incoming_badge[:value] < 180
      stored_badge.update(ranking: "Elite", ranking_value: 3, value: incoming_badge[:value])
    else
      #Raise error?
    end
  end
